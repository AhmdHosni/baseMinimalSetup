...
function yi() {
	...
	# Start yazi with the chooser flag
	yazi "$@" --chooser-file="$tmp"
	
	# If a file was chosen (not just quitting with q)
	if chosen="$(cat -- "$tmp")" && [ -n "$chosen" ]; then
		# clear
		# Run kitty's image viewer on the chosen path
		# The --hold flag keeps the image on screen until you press a key
		kitten icat --hold "$chosen"
	fi
	# rm -f -- "$tmp"
    # Read the file and change directory if it's different
    if [ -f "$tmp" ]; then
...
function yq() {
    ...
    local selected_file="${tmp}-file"

    # Run yazi
    yazi "$@" --cwd-file="$tmp" --chooser-file="$selected_file"

    # 1. Handle Directory Change (CWD)
    if [ -f "$tmp" ]; then
        cwd="$(cat -- "$tmp")"
        if [ -n "$cwd" ] && [ "$cwd" != "$PWD" ]; then
            builtin cd -- "$cwd"
    ...
    # 2. Handle Image Preview (only if a file was selected)
    if [ -f "$selected_file" ]; then
        chosen="$(cat -- "$selected_file")"
        if [ -n "$chosen" ]; then
            clear
            echo "Viewing: $(basename "$chosen")"
            echo "Path: $chosen"
            echo "--------------------------------"
...
function chooseOnQuit() {
    local tmp="$(mktemp -t "yazi-cwd.XXXXXX")"
    local selected_file="${tmp}-file"

    # Run yazi with both flags
    yazi "$@" --cwd-file="$tmp" --chooser-file="$selected_file"
    ...
    # Show image ONLY if the selection file exists and isn't empty
    if [ -f "$selected_file" ] && [ -s "$selected_file" ]; then
        ...
        # If no file was chosen, just clean up quietly
        rm -f "$selected_file"
    fi
}

function yx() {
    local tmp_cwd="$YAZI_DIR/yazi-cwd.tmp"
    local tmp_file="$YAZI_DIR/yazi-selected.tmp"
    
    # Run yazi with BOTH flags
    yazi "$@" --cwd-file="$tmp_cwd" --chooser-file="$tmp_file"
    
    # 1. Handle Directory Change
    if [ -f "$tmp_cwd" ]; then
        local cwd="$(cat -- "$tmp_cwd")"
        if [ -n "$cwd" ] && [ "$cwd" != "$PWD" ]; then
            cd -- "$cwd"
        fi
        rm -f -- "$tmp_cwd"
    ...
    # 2. Handle Image Output (The part you were missing)
    if [ -f "$tmp_file" ]; then
        local chosen="$(cat -- "$tmp_file")"
        if [ -n "$chosen" ]; then
            clear
            echo "File: $(basename "$chosen")"
            echo "Path: $chosen"
            echo "--------------------------------"
            kitten icat "$chosen"
        fi
